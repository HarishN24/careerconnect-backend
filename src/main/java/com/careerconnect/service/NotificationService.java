...
public class NotificationService {
	...
	@Value("${resend.from-email:noreply@careerconnect.com}")
	...
	public void sendApplicationNotification(Application application) {
		...
		send(recruiterEmail, "New Job Application - " + jobTitle,
	...
	public void sendApplicationConfirmation(Application application) {
		...
		send(applicantEmail,
				...
				"Hello " + applicantName + ",\n\n"
						...
						+ "get in touch with you if your profile is " + "shortlisted for the next stage.\n\n"

						+ "You can track your application status anytime "
						+ "by logging in to your CareerConnect account.\n\n"
	...
	public void sendStatusUpdateNotification(Application application) {
		...
		send(applicantEmail,
				...
				"Hello " + applicantName + ",\n\n"
						...
						+ "We wanted to let you know that there has " + "been an update to your application for the "
						+ jobTitle + " position at " + companyName + ".\n\n"

						+ "Your current application status is: " + status + ".\n\n"

						+ "Please log in to your CareerConnect account " + "to view the latest details regarding your "
						+ "application.\n\n"

						+ "Thank you for your interest and patience. " + "We wish you the best in the next steps of "
	...
	private void send(String to, String subject, String text) {
		...
		if (!StringUtils.hasText(fromEmail)) {
			log.warn("MAIL_FROM is not configured - " + "skipping email to {}", to);
			return;
		}
		...
		try {
			...
			Map<String, Object> payload = Map.of("from", fromEmail, "to", List.of(to), "subject", subject, "text",
					text);

			String body = objectMapper.writeValueAsString(payload);

			HttpRequest request = HttpRequest.newBuilder().uri(RESEND_ENDPOINT).timeout(Duration.ofSeconds(10))
					.header("Authorization", "Bearer " + apiKey).header("Content-Type", "application/json")
					.header("User-Agent", "CareerConnect/1.0").POST(HttpRequest.BodyPublishers.ofString(body)).build();
			...
			if (response.statusCode() >= 200 && response.statusCode() < 300) {
				...
				log.error("Resend API returned {} for recipient {}: {}", response.statusCode(), to, response.body());
			}
